defmodule TimeManager.Workingtimes do
  ...
  def get_workingtimesWithUserId(user_id) do
    Workingtime
    |> where(user_id: ^user_id)
    |> Repo.all()
  end

  def get_workingtimeWithUserId(user_id, workingtime_id) do
    Workingtime
    |> where(user_id: ^user_id)
    |> Repo.get(workingtime_id)
  end

  def get_user_workingtime(user_id) do
    Workingtime
    |> where([w], w.user_id == ^user_id)
    |> Repo.all()
  end

  def get_workingtime_startDate_endDate(id, startDate, endDate) do
    Workingtime
    |> where([w], w.user_id == ^id and w.start >= ^startDate and w.end <= ^endDate)
    |> Repo.all()
  end

  def get_workingtime_startDate(id, startDate) do
    Workingtime
    |> where([w], w.user_id == ^id and w.start >= ^startDate)
    |> Repo.all()
  end

  def get_workingtime_endDate(id, endDate) do
    Workingtime
    |> where([w], w.user_id == ^id and w.end <= ^endDate)
    |> Repo.all()
  end
