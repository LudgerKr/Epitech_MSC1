      ...
      <b-navbar v-if="$route.path !=='/'" type="dark" variant="warning" class="navbar-horizontal" style="max-height: 50%">
        <b-navbar-nav class="align-center">
          ...
          <b-nav-item-dropdown>
            ...
            <b-dropdown-item @click="$bvModal.show('bv-modal-profile')">
          ...
          <b-modal id="bv-modal-profile" title="User" hide-footer>
            ...
            <b-tab title="Edit">
              ...
              <b-button-group right>
                <b-button variant="dark" @click="$bvModal.hide('bv-modal-profile')">
                  <b-icon icon="x-circle"></b-icon>
                  Cancel
                </b-button>
                <b-button variant="success" @click="edit_user()">
                  <b-icon icon="check"></b-icon>
                  Edit
                </b-button>
                <b-button variant="danger" @click="delete_user()">
          ...
          <b-modal id="bv-modal-analytic" title="Analytic" hide-footer>
            <b-form>
            ...
            <b-button-group right>
              <b-button variant="dark" @click="$bvModal.hide('bv-modal-analytic')">
                <b-icon icon="x-circle"></b-icon>
                Cancel
              </b-button>
              <b-button variant="success" :user="user" @child-props="getUser" @click="go_analytic_admin()">
                <b-icon icon="check"></b-icon>
                Validate
              </b-button>
            </b-button-group>
          ...
          <b-modal id="bv-modal-planing" title="Planing" hide-footer>
            <b-button-group align >
              <b-button squared style="background-color: #e85d04; border-color:#e85d04" :user="user" @child-props="getUser" @click="goPlaning()">
                My schedule
              </b-button>
              <b-button squared style="background-color: #d00000; border-color:#d00000;" v-if="user.role_id == 1" :user="user" @child-props="getUser" @click="create_planing()">
...
export default {
  ...
  mounted() {
    axios.all([
      axios.get('http://localhost:4001/api/v1/profile/' + this.$router.currentRoute.params.id, config),
  ...
  methods:{
    ...
    // Delete user
    delete_user(){
      axios.delete('http://127.0.0.1:4001/api/v1/profile/'+ this.user.id, config);
      this.$router.push('/');
    },
    //edit user
    edit_user() {
      axios.put('http://127.0.0.1:4001/api/v1/profile/' + this.user.id, {
        "user":{
    ...
    setCheckIn(){
      ...
      if (this.clickIn === 1) {
        ...
        axios.post('http://127.0.0.1:4001/api/v1/clocks/' + this.user.id, {
          "clock": {
            time: this.timeIn,
            status: true
          }}, config);
