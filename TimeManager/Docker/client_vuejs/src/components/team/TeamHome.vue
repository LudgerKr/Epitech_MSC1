<template>
    <b-container fluid class="bv-example-row bv-example-row-flex-cols">
      ...
      <router-link :to="{ name: 'addMemberTeam', params: {team_id: team.id} }">
        <b-button :user="user" @child-props="getUser">Add new member</b-button>
      </router-link>
      <br>
      <br>
      <router-link :to="{ name: 'editTeam', params: {team_id: team.id} }">
        <b-button :user="user" @child-props="getUser">Change Team's name</b-button>
      </router-link>
      <b-row>
        <v-col v-for="(member, index) in users" :key="index" cols="3">
          ...
          <router-link :to="{ name: 'clocksUser', params: {user_id: member.id} }">
            <b-button v-if="role_id!=3"  :user="user" @child-props="getUser">Clocks</b-button>
          </router-link>
          <router-link :to="{ name: 'WorkingTimes', params: {user_id: member.id} }">
            <b-button v-if="role_id!=3"  :user="user" @child-props="getUser">Workingtimes</b-button>
          </router-link>
          <router-link :to="{ name: 'AnalyticUser', params: {id: member.id} }">
            <b-button v-if="role_id!=3"  :user="user" @child-props="getUser">Analytics</b-button>
          </router-link>
        </v-col>
        <b-col cols="8">
        </b-col>
...
export default {
  ...
  mounted () {
    axios.all([
      axios.get('http://13.68.159.231:4001/api/manager/users/teams/' + this.$router.currentRoute.params.team_id, config),
