<template>
  <div>
      <h1>WorkingTimes</h1>
      <v-col v-for="(workingTime, index) in workingTimes" :key="index">
        ...
        <router-link :to="{ name: 'WorkingTimes' }">
          <v-btn :user="user" @child-props="getUser" @click="supress(workingTime.id)">
            <span>Delete</span>
          </v-btn>
        </router-link>
      </v-col>
...
export default {
  ...
  mounted() {
    axios.all([
      axios.get('http://13.68.159.231:4001/api/v1/workingtimes/' + this.$router.currentRoute.params.user_id, config),
  ...
  methods: {
    supress(workingtime_id) {
      axios.delete('http://13.68.159.231:4001/api/admin/workingtimes/' + workingtime_id, config);
    },
    parseDate(date) {
